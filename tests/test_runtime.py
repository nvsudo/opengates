...
def test_http_thread_routes_work(monkeypatch, tmp_path) -> None:
    ...
    assert "Filtering for high-signal founder inbound" in response.text
...
def test_public_path_form_aliases_work(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("OPENGATES_CONFIG_DIR", str(tmp_path))
    monkeypatch.delenv("OPENGATES_GATES_DIR", raising=False)
    monkeypatch.delenv("OPENGATES_DATA_DIR", raising=False)

    client = TestClient(create_app())

    intake = client.get("/demo")
    assert intake.status_code == 200

    create = client.post(
        "/demo/threads",
        data={"name": "Founder", "email": "alias@example.com", "content": "We are building applied AI software."},
        follow_redirects=False,
    )
    assert create.status_code == 303
    assert create.headers["location"].startswith("/t/")


def test_settings_fall_back_to_bundled_gates(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("OPENGATES_CONFIG_DIR", str(tmp_path))
    monkeypatch.delenv("OPENGATES_GATES_DIR", raising=False)
    monkeypatch.delenv("OPENGATES_DATA_DIR", raising=False)
    ...
    assert settings.gates_dir == settings.bundled_gates_dir
    assert settings.bundled_gates_dir == Path(__file__).resolve().parents[1] / "src" / "opengates" / "starter_gates"
