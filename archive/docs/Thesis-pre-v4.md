...
- Naming: **Ante** is the commercial product brand. **OpenGates** is the open-source runtime. This document uses "OpenGates" for the runtime and "Ante" for the commercial product.
...
The default commercial product (Ante) should be a hosted, thread-first gate conversation:
...
The principal controls the economics of their gates. Options include free (tokens absorbed), flat charges (token offset and noise deterrence), priority lanes, or auctions. Payment is a seriousness filter, not a pass. Charging may also be used to fund deeper multi-turn diligence, but the gate should never equate payment with quality.
