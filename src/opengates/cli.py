...
def main() -> None:
    ...
    if args.command == "init-gate":
        source_path = resolve_source_gate_path(settings, args.source_gate)
        target_root = settings.user_gates_dir
        target_root.mkdir(parents=True, exist_ok=True)
        target_path = target_root / args.target_gate
...
def resolve_source_gate_path(settings, source_gate: str) -> Path:
    candidate_roots = [settings.gates_dir, settings.user_gates_dir, settings.bundled_gates_dir]
    seen: set[Path] = set()
    for root in candidate_roots:
        ...
        if source_path.exists():
            return source_path
    return settings.gates_dir / source_gate
